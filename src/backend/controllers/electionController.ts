...
import { PrismaClient, ElectionStatus, CandidateStatus } from '@prisma/client';
import { z } from 'zod';
...
export const getElections = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      count: formattedElections.length,
...
export const castVote = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(201).json({
      success: true,
      message: 'Your vote has been recorded successfully. Thank you for participating in the election.',
      data: {
        voteId: vote.id,
    ...
    return res.status(500).json({
      success: false,
      message: 'An error occurred while recording your vote. Please try again later.',
      error: error instanceof Error ? error.message : 'Unknown error'
    });
  }
};
...
export const getElectionResults = async (req: Request, res: Response) => {
  try {
    ...
    // Results only available for completed elections unless admin/faculty
    const userRole = req.user?.role || 'STUDENT';
    
    if (election.status !== 'COMPLETED' && userRole === 'STUDENT') {
      return res.status(403).json({
        success: false,
        message: 'Election results are only available after the election has completed.'
      });
    }
    ...
    // Format results for accessibility
    const formattedResults = {
      ...
      candidates: candidates.map(candidate => ({
        ...
        // Calculate percentage for accessible charts
        votePercentage: totalVotes > 0 ? Math.round((candidate.voteCount / totalVotes) * 100) : 0,
        studentName: candidate.student?.name,
        department: candidate.student?.department
      }))
    };
...
function getStatusDescription(status: ElectionStatus): string {
  switch (status) {
    case 'UPCOMING':
      return 'This election is upcoming and has not started yet.';
    case 'ACTIVE':
      return 'This election is currently active and accepting votes.';
    case 'COMPLETED':
      return 'This election has completed. Results are available.';
    case 'CANCELLED':
...
function getCandidateStatusDescription(status: CandidateStatus): string {
  switch (status) {
    case 'PENDING':
      return 'Your application is pending review by administrators.';
    case 'APPROVED':
      return 'Your application has been approved. You will be included in the election.';
    case 'REJECTED':
      return 'Your application has been rejected. Contact administrators for more information.';
    default:
      return '';
  }
}
